...
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 3 >>>>>>>>>>>>>>>>>>>>>>>
-- Do an inner join to get only the rows from the movies tables that have matching rows in the users tables.
...
SELECT lead_studio, count(lead_studio)FROM movies INNER JOIN users on movie_id = favorite_movie_id
group by lead_studio
order by count(lead_studio) desc
limit 1;
